...
def test_create_user_and_get_by_email_and_id(db_conn):
    created = create_user(db_conn, "User@Example.com", "hashed-password")
...
def test_create_user_duplicate_email_raises(db_conn):
    create_user(db_conn, "user@example.com", "hash-one")

    with pytest.raises(DuplicateEmailError):
        create_user(db_conn, "USER@example.com", "hash-two")
